        ...
        <Card className="lg:col-span-1 border-slate-700 bg-slate-800/50 relative overflow-hidden">
          ...
          <CardContent className="flex flex-col items-center justify-center py-8">
            ...
            <div className="mt-6 text-center">
              <Badge variant={data.riskLevel === 'High' ? 'danger' : data.riskLevel === 'Medium' ? 'warning' : 'success'} className="mb-2 text-sm px-3 py-1">
                {data.riskLevel.toUpperCase()} RISK
              </Badge>
              <p className="text-slate-300 text-sm mt-2">{data.recommendation}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-between mb-6 p-4 rounded-lg bg-slate-900 border border-slate-800">
              ...
              <Badge variant={data.tampering.overall === 'Medium' ? 'warning' : 'danger'}>{data.tampering.overall}</Badge>
            </div>
            ...
            <div className="mt-6 h-32 bg-slate-800 rounded-lg border border-slate-700 flex items-center justify-center overflow-hidden relative">
              ...
              <span className="text-slate-500 relative z-10 flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-red-500" /> Text region requires review
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4 mb-6">
              <div className="flex items-start gap-3 text-amber-400 text-sm">
                ...
                <div>
                  <div className="font-medium mb-1">Relationship inconsistency</div>
                  <div className="text-amber-500/80">{data.relationships.explanation}</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-4 py-4 bg-slate-900 rounded-lg border border-slate-800">
              ...
              <div className="text-xs text-slate-500 bg-slate-950 px-2 py-0.5 rounded border border-slate-800 relative -my-3 z-10">Mother</div>
              <div className="h-4 w-px bg-slate-700"></div>
              ...
              <div className="text-xs text-red-400 bg-red-500/10 px-2 py-0.5 rounded border border-red-500/20 relative -my-3 z-10">Son</div>
              <div className="h-4 w-px bg-red-500/50"></div>
              <div className="bg-red-500/10 border border-red-500/30 text-red-400 px-4 py-2 rounded-lg text-sm flex items-center gap-2">
                Rahul Rao <AlertCircle className="w-4 h-4" />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              {data.intelligence.contributions.map((c, i) => (
                <div key={i} className="flex justify-between items-center text-sm border-b border-slate-800/50 pb-2">
                  ...
                  <span className={`font-mono font-medium ${c.value.startsWith('+') ? 'text-red-400' : 'text-emerald-400'}`}>{c.value}</span>
                </div>
              ))}
            </div>
            <div className="flex flex-col items-center justify-center bg-slate-900/50 rounded-xl border border-slate-800 p-6">
              ...
              <div className="text-5xl font-bold text-white mb-2">{data.riskScore}</div>
              <div className="text-sm text-slate-400 mb-4">Confidence: {data.intelligence.confidence}%</div>
